...
struct RenderBase {
    ...
    RenderBase(bool headless, uint32_t w, uint32_t h, const char * title) {
        instance = std::make_unique<Instance>(Instance::ConstructParameters {}.setValidation(Instance::BREAK_ON_VK_ERROR).setBacktrace(backtrace));
        device   = std::make_unique<Device>(Device::ConstructParameters {*instance}.setPrintVkInfo(Device::BRIEF));
        commandQueue =
            Ref<CommandQueue>::make(CommandQueue::ConstructParameters {{"main"}, device->gi(), device->graphics()->family(), device->graphics()->index()});
        width  = w;
        height = h;
        if (!headless) {
            glfwInit();
        ...
        swapchain = Ref<Swapchain>::make(Swapchain::ConstructParameters {{"swapchain"}}
                                             ...
                                             .setDevice(*device)
                                             .setDimensions(headless ? w : 0, headless ? h : 0));
    }

    ~RenderBase() {
        swapchain.reset(); // must destroy swapchain before destroying the surface.
        if (surface) instance->handle().destroySurfaceKHR(surface), surface = VK_NULL_HANDLE;
